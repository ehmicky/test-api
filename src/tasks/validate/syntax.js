...
export const validateTasksSyntax = ({ tasks }) => {
  if (tasks.length === 0) {
...
const validateDuplicateKeys = ({ tasks }) => {
  tasks.forEach(validateDuplicateKey)
}

const validateDuplicateKey = ({ key, scope, name }, index, tasks) => {
